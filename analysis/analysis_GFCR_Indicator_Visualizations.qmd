...
```{r}
#| label: F4 plots for benthic cover
...
```{r}
#| label: F7.3 plot - financial mechanisms
gfcrIndF7_3PlotTBL <- gfcrProjectReports$F7 %>% 
  filter(`Data Type` == "Report" &
           grepl(pattern = "F7.3", x = `Sub-Indicator Name`)) %>% 
...
dataLabels = gfcrIndF7_3PlotTBL$Label
names(dataLabels) <- gfcrIndF7_3PlotTBL$Color

ggplot() +
  geom_col(data = gfcrIndF7_3PlotTBL %>% 
             filter(Color == "Report"),
           aes(x = `Reporting Date`, y = Value, fill = Color),
           width = 0.25) +
  geom_hline(data = gfcrIndF7_3PlotTBL %>% 
               filter(Color != "Report"),
             aes(yintercept = Value, color = Color),
             linetype = "dashed",
             linewidth = 1) +
  labs(x = "",
       y = "# financial mechanisms/reforms",
       title = "F7.3 Number of financial mechanisms/reforms") +
  scale_fill_manual(values = dataColors) +
  scale_color_manual(values = dataColors, labels = dataLabels) +
  guides(fill = "none") +
  theme(aspect.ratio = 0.5,
...
```{r}
#| label: F7.4 plot - governance reforms/policies
gfcrIndF7_4PlotTBL <- gfcrProjectReports$F7 %>% 
  filter(`Data Type` == "Report" &
           grepl(pattern = "F7.4", x = `Sub-Indicator Name`)) %>% 
  bind_rows(gfcrProjectReports$F7 %>% 
              filter(`Data Type` == "Target" &
                       grepl(pattern = "F7.4", x = `Sub-Indicator Name`)) %>% 
              filter(`Reporting Date` == max(`Reporting Date`))) %>% 
...
dataLabels = gfcrIndF7_4PlotTBL$Label
names(dataLabels) <- gfcrIndF7_4PlotTBL$Color

ggplot() +
  geom_col(data = gfcrIndF7_4PlotTBL %>% 
             filter(Color == "Report"),
           aes(x = `Reporting Date`, y = Value, fill = Color),
           width = 0.25) +
  geom_hline(data = gfcrIndF7_4PlotTBL %>% 
               filter(Color != "Report"),
             aes(yintercept = Value, color = Color),
             linetype = "dashed",
             linewidth = 1) +
  labs(x = "",
       y = "# governance reforms/policies",
       title = "F7.4 Number of governance reforms/policies") +
  scale_fill_manual(values = dataColors) +
  scale_color_manual(values = dataColors, labels = dataLabels) +
  guides(fill = "none") +
  theme(aspect.ratio = 0.5,
